...
if(isset($_POST['submit']))
{
	...
	if(($name!="")&&($email!="")&&($password!="")&&($password=$cpassword)&&($terms==true))
	{
		...
		if($parent=="upendrasingh370@gmail.com")
		{
			if($num2<2)
		{
			if($caluser==0)
	...
	$id=time();
	echo $id;
	  	$query="insert into `waiting` values('$email','$name','$phone','$address','$date','$password',0,'$parent','$id')";
		mysql_query($query);
$subject = 'Email verification of your account';
$message = 'hello '.$name." your details were submitted successfully please click on the link to verify your email\n"."www.umcmillsverify.com?id=".$id;
$headers = 'From: upendrasingh370@gmail.com' ;

mail($email, $subject, $message, $headers);
        echo "<div class=row style=\"margin-top:200px;\"><h3>Hello ".$name." you are done Your Request Was successfuly submitted please verify your email</h3></div>";
			}
			else
			{
		...
		{
			$msg="sorry your account can not be created as the selected upliner already has 2 downliner select some other downliner";
			//header("location:acknowledgement.php?id='$msg'");
			echo "<h3>$msg</h3>";
		}
		}
...
mail($email, $subject, $message, $headers);
        echo "<div class=row style=\"margin-top:200px;\"><h3>Hello ".$name." you are done Your Request Was successfuly submited please verify your email</h3></div>";
			}
			else
			{
				$msg="Sorry ".$name." the user with that email Already exists";
