...
function find_downliner($node)
{
	if($node!="")
	{
		...
		$dquery="select * from `user` where u_parent='$node' AND u_approved=1";
...
static $parent=array();
	if($node!="NULL")
	{
		$query="select * from `user` where u_email='$node' AND u_approved=1";
		$result=mysql_query($query);
...
<a href="updatedetails.php?id=<?php echo $email; ?>"><button class="btn btn-primary">Update Details</button></a>
...
<td>
<a href="approve.php?id=<?php echo $arow['u_email']; ?>"><button class="btn btn-primary"> approve</button></a>
</td>
</tr>
<?php
}
...
<button class="btn btn-danger" name="logout">Logout(<?php echo $name; ?>)</button>
</form>
<?php
